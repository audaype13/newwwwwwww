...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "suggest_quote":
        ...
        keyboard = [
            [InlineKeyboardButton(label, callback_data=f"suggest_cat_{value}")]
    ...
    if data.startswith("approve_quote_"):
        if user_id != config.DEVELOPER_ID:
            return
        quote_id = int(data[len("approve_quote_"):])
    ...
    if data.startswith("reject_quote_"):
        if user_id != config.DEVELOPER_ID:
            return
        quote_id = int(data[len("reject_quote_"):])
    ...
    if data == "manage_channels":
        ...
        # جلب البيانات من DB أولاً ثم إغلاق الـ session
        ...
        # فلترة القنوات بعد إغلاق الـ session
        accessible_channels = []
    ...
    if data.startswith("edit_channel_") and data != "edit_channel_time":
        try:
            ch_id = int(data[len("edit_channel_"):])
        except ValueError:
            return
    ...
    if data == "confirm_del_channel":
        ...
        keyboard = [
            ...
            [InlineKeyboardButton("✅ نعم، احذف", callback_data=f"delete_channel_{ch_id}")]
        ]
        await query.edit_message_text(
    ...
    if data.startswith("delete_channel_"):
        ch_id = int(data[len("delete_channel_"):])
        title = None
        with db.get_db_session() as session:
            ch = session.query(db.Channel).filter_by(id=ch_id).first()
            if ch:
    ...
    if data == "manage_admins":
        ...
        keyboard = [
            ...
            [InlineKeyboardButton("➖ إزالة مشرف", callback_data="del_admin_step1")],
    ...
    if data == "bot_settings":
        ...
        keyboard = [
            [InlineKeyboardButton("📢 قناة البوت", callback_data="set_bot_channel")],
            [InlineKeyboardButton("ℹ️ نص عنا", callback_data="set_bot_about")],
            [InlineKeyboardButton("👋 رسالة الترحيب", callback_data="set_welcome_msg")],
            [InlineKeyboardButton("🔒 قناة اشتراك إجباري", callback_data="set_force_sub")],
            [
                InlineKeyboardButton("✏️ تعديل إعداد", callback_data="edit_setting_prompt"),
                InlineKeyboardButton("➕ إضافة إعداد", callback_data="add_setting_prompt"),
            ],
            [InlineKeyboardButton("🔙 رجوع", callback_data="back_dev")],
        ]
    ...
    if data == "backup_menu":
        if user_id != config.DEVELOPER_ID:
            return
        keyboard = [
            [InlineKeyboardButton("💾 تصدير نسخة احتياطية", callback_data="backup_export")],
            [InlineKeyboardButton("📥 استعادة نسخة احتياطية", callback_data="backup_import_prompt")],
            [InlineKeyboardButton("🔙 رجوع", callback_data="back_dev")],
        ]
        with db.get_db_session() as session:
    ...
    if data.startswith("preview_ch_"):
        ch_id = int(data[len("preview_ch_"):])
        with db.get_db_session() as session:
            ch = session.query(db.Channel).filter_by(id=ch_id).first()
            if not ch:
    ...
    if data.startswith("toggle_channel_"):
        ch_id = int(data[len("toggle_channel_"):])
    ...
    if data == "post_now":
        ...
        try:
            ...
            await query.edit_message_text(f"❌ <b>فشل النشر</b>\n<code>{e}</code>", parse_mode='HTML', reply_markup=get_back_keyboard(role))
