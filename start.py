...
async def broadcast_task(context, text):
    ...
    logger.info(f"Broadcast done. Users: {success_users}, Channels: {success_channels}")
    try:
...
async def message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get('action') == 'waiting_user_quote':
        ...
        kb = InlineKeyboardMarkup([
            [
                InlineKeyboardButton("✅ قبول", callback_data=f"approve_quote_{quote_id}", style="success"),
                InlineKeyboardButton("🚫 رفض", callback_data=f"reject_quote_{quote_id}", style="danger"),
            ]
        ])
